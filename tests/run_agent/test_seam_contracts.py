...
import model_tools
import run_agent
from run_agent import AIAgent, _PHASE_PRIVATE_MARKER, _public_only
...
def test_contract_public_only_filter_behavior():
    ...
    assert callable(_public_only)

    pub_msg = {"role": "user", "content": "public user message"}
    priv_msg = {"role": "assistant", "content": "private thought", _PHASE_PRIVATE_MARKER: True}

    filtered = _public_only([pub_msg, priv_msg])
    assert len(filtered) == 1
    assert filtered[0] == pub_msg
    assert _PHASE_PRIVATE_MARKER not in filtered[0]
...
def test_contract_message_persistence_flows_through_unlocked_flush():
    ...
    assert hasattr(AIAgent, "_flush_messages_to_session_db_unlocked")
    sig = inspect.signature(AIAgent._flush_messages_to_session_db_unlocked)
    ...
    priv_msg = {"role": "assistant", "content": "private secret", _PHASE_PRIVATE_MARKER: True}
...
def test_contract_save_trajectory_uses_public_only_filter(tmp_path):
    ...
    assert hasattr(AIAgent, "_save_trajectory")
    ...
    priv_msg = {"role": "assistant", "content": "private trajectory item", _PHASE_PRIVATE_MARKER: True}

    with patch("run_agent._save_trajectory_to_file", side_effect=lambda t, m, c: save_trajectory(t, m, c, str(traj_file))):
...
def test_contract_fail_red_demonstration():
    ...
    with patch("run_agent._public_only", side_effect=lambda msgs: msgs):
        priv_msg = {"role": "assistant", "content": "private", _PHASE_PRIVATE_MARKER: True}
        result = run_agent._public_only([priv_msg])
        assert len(result) == 1
        assert _PHASE_PRIVATE_MARKER in result[0]
