...
def _setup_journal_env(home_dir):
    ...
    priv_key, pub_key = sign.generate_signing_key()
    (hermes_dir / "wharenui_sign.key").write_bytes(priv_key)
...
def test_b8_2_seam_states(temp_home):
    # This test will subprocess for absent state and mock for others
    import subprocess
    env = os.environ.copy()
...
print("seam state:", wharenui_plugin.get_seam_state())
    ...
    assert "seam state: absent" in res.stdout, f"Absent state failed: {res.stdout}"
